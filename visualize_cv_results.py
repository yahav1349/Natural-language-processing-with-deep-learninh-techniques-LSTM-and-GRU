...
def plot_params_barplot(data, figsize=(16, 8), top_n=None, sort_by_score=True):
    ...
    # Color bars based on score (gradient)
    colors = plt.cm.viridis((df['mean_score'] - df['mean_score'].min()) / 
    ...
    sm = plt.cm.ScalarMappable(cmap='viridis', 
                               norm=plt.Normalize(vmin=df['mean_score'].min(), 
                                                 vmax=df['mean_score'].max()))
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, label='Accuracy', pad=0.01)
...
def plot_mean_scores_comparison(data, figsize=(16, 6)):
    ...
    colors1 = plt.cm.viridis((means1 - vmin) / (vmax - vmin))
    ...
    colors2 = plt.cm.viridis((means2 - vmin) / (vmax - vmin))
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax2, label='Accuracy', pad=0.02)
